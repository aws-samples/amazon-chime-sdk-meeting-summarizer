  ...
  try {
    ...
    if (transcript) {
      ...
      const processedChunks = await Promise.all(chunks.map(async (chunk) => {
        const prompt = createPayload(chunk);
      ...
      const cleanedTranscript = processedChunks.join(' ');
      await writeBucket(latestObjectKey, cleanedTranscript, PREFIX || 'clean-transcript');
...
const splitTranscript = (transcript: string, maxTokens: number): string[] => {
  const chunks: string[] = [];
  let startIndex = 0;
  let endIndex = maxTokens;
  while (startIndex < transcript.length) {
    // Ensure we don't split in the middle of a speaker's turn
    const nextSpeakerTurn = transcript.indexOf('\n', endIndex);
    if (nextSpeakerTurn > startIndex && nextSpeakerTurn < startIndex + maxTokens) {
      endIndex = nextSpeakerTurn + 1;
    }
    chunks.push(transcript.slice(startIndex, endIndex).trim());
    startIndex = endIndex;
